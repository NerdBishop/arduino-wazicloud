...
void loop() {
  //XXXX IS THE FIRST SENSOR'S ID - example temperatureSensor_1
  send_to_cloud("XXXX/value", 20); //SENDING SENSOR 1 DATA e.g sending a dummy value of 23

  delay(500);

  //YYYY IS THE SECOND SENSOR'S ID - example humiditySensor_1
  send_to_cloud("YYYY/value" , 60); //SENDING SENSOR 2 DATA e.g sending a dummy value of 25
...
void send_to_cloud(String u, int data) {
  ...
  if (WiFi.status() == WL_CONNECTED) { //CHECK WIFI CONNECTION STATUS
    ...
    int value = data; //DUMMY SENSOR VALUE, REPLACE WITH YOUR ACTUAL SENSOR DATA
    char body[20]; //HOLDS JSON DATA
    sprintf( body, "{\"value\": %d}", value); //FORMATTING DATA
